    ...
    <x-slot name="header">
        <div>
            <div class="flex items-center gap-2 text-sm text-gray-500">
                ...
                <span class="text-gray-900 font-medium">Edit #{{ $reservation->id }}</span>
            </div>
            <h2 class="mt-1 text-xl font-bold text-gray-900">Edit Booking #{{ $reservation->id }}</h2>
        </div>
    </x-slot>

    <div class="py-8 animate-fade-in">
        <div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
            <form method="POST" action="{{ route($isAdminView ? 'admin.reservations.update' : 'reservations.update', $reservation) }}" class="card p-6 space-y-6">
                @csrf
                @method('PUT')
                @include('reservations._form')
                <div class="flex items-center justify-end gap-3 border-t border-gray-100 pt-6">
                    <a href="{{ route($isAdminView ? 'admin.reservations.show' : 'reservations.show', $reservation) }}" class="btn-secondary">Cancel</a>
                    <x-primary-button>
