<x-app-layout>
    <x-slot name="header">
        <div>
            <div class="flex items-center gap-2 text-sm text-gray-500">
                <a href="{{ route('admin.chambres.index') }}" class="hover:text-blue-600 transition-colors">Rooms</a>
                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" /></svg>
                <span class="text-gray-900 font-medium">Edit Room {{ $chambre->numero }}</span>
            </div>
            <h2 class="mt-1 text-xl font-bold text-gray-900">Edit Room {{ $chambre->numero }}</h2>
        </div>
    </x-slot>

    <div class="py-8 animate-fade-in">
        <div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('admin.chambres.update', $chambre) }}" class="card p-6 space-y-6">
                @csrf
                @method('PUT')
                @include('chambres._form')
                <div class="flex items-center justify-end gap-3 border-t border-gray-100 pt-6">
                    <a href="{{ route('admin.chambres.index') }}" class="btn-secondary">Cancel</a>
                    <x-primary-button>
                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                        Save Changes
                    </x-primary-button>
                </div>
            </form>

            <form method="POST" action="{{ route('admin.chambres.destroy', $chambre) }}" class="mt-6 text-right" onsubmit="return confirm('Delete this room and all its reservations? This cannot be undone.')">
                @csrf
                @method('DELETE')
                <button class="btn-danger">
                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" /></svg>
                    Delete Room
                </button>
            </form>
        </div>
    </div>
</x-app-layout>
